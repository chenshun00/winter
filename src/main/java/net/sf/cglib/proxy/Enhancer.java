...
public class Enhancer extends AbstractClassGenerator {
     ...
     * and the latter enables faster start than plain old reflection lookup and invoke.
    ...
    @Override
    protected Object nextInstance(Object instance, Object key) throws Exception {
        EnhancerFactoryData data = (EnhancerFactoryData) instance;
        if (classOnly) {
            return data.generatedClass;
        }
        ...
        return data.newInstance(argumentTypes, arguments, callbacks, key);
    }
    ...
    public interface EnhancerKey {
        Object newInstance(String type, String[] interfaces, WeakCacheKey<CallbackFilter> filter, Type[] callbackTypes,
                           boolean useFactory,
    ...
    static class EnhancerFactoryData {
        ...
        public Object newInstance(Class[] argumentTypes, Object[] arguments, Callback[] callbacks, Object key) {
            setThreadCallbacks(callbacks);
            try {
                // Explicit reference equality is added here just in case Arrays.equals does not have one
                if (primaryConstructorArgTypes == argumentTypes ||
                    ...
                    // If we have relevant Constructor instance at hand, just call it
                    // This skips "get constructors" machinery
                    Object instance = ReflectUtils.newInstance(primaryConstructor, arguments);
                    try {
                        Field[] fields = instance.getClass().getSuperclass().getDeclaredFields();
                        for (Field field : fields) {
                            field.setAccessible(true);
                            Object o = field.get(key);
